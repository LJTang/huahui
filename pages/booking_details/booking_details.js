...
Page({
    ...
    onMsgCallBack_Stay:function (jsonBack){
        ...
        if(data.code==200){
            ...
            if(Date.parse(new Date())<data.data.order_detail.end_time){
                this.countdown(data.data.order_detail.end_time);
                this.setData({
                    times_Bool:true
                });
            }else{
    ...
    countdown:function(time){
        ...
        this.data.intervarID= setInterval(function () {
            ...
            var times =time-timestamp;
            // var leftTime = (new Date(2018, 8-1, 28, 17, 40, 0)) - (new Date()); //计算剩余的毫秒数
            console.log(times);
            var days = parseInt(times / 1000 / 60 / 60 / 24, 10); //计算剩余的天数
            var hours = parseInt(times / 1000 / 60 / 60 % 24, 10); //计算剩余的小时
